...
def run_solver_auth_check(rpc_url, direct, to_addr, sender, landed_block,
    ...
    (tx.to) for a solver-owned contract. `status_ok` guards against a false
    accusation: a SUCCESSFUL settle() provably passed GPv2's onlySolver gate, so
    a negative reading on a succeeded tx can only be a reconstruction artifact
    (e.g. the solver was de-registered after the fact, or a non-archive RPC
    answered from latest state) → UNCERTAIN, never VIOLATION."""
    ...
    if ok is None:
        try:
            ...
            checks.append(_v("C9.solver-authorization", UNCERTAIN,
                             f"authenticator query failed: {str(e)[:80]}"))
            return
